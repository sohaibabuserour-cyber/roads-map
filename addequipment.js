...
function openEquipmentFormModal() {
    ...
    if (typeof eqSwitchFormTab === 'function') eqSwitchFormTab('daily');
...
function eqCancelPickFromMap() {
    ...
    const scr = document.getElementById('additionScreen');
    if (scr) { scr.style.removeProperty('display'); scr.style.display = 'flex'; }
...
/* ── Tab switching — المعدات اليومية فقط؛ التراكمي فى تبويب منفصل ── */
function eqSwitchFormTab(tab) {
    if (tab === 'cumulative') {
        if (typeof window.openCumulQtyTab === 'function') window.openCumulQtyTab();
        return;
    }
    if (tab === 'target') {
        if (typeof openTargetFormTab === 'function') openTargetFormTab();
        return;
    }
...
function eqInitCumulTab() {
    eqBuildElementsList();
    eqPopulateContractors();
    const today = new Date().toISOString().split('T')[0];
    const d = document.getElementById('eqfc_date');
    if (d && !d.value) d.value = today;
}

function eqResetCumulForm() {
    eqClearCumulElement();
    ['eqfc_contractor', 'eqfc_total_qty', 'eqfc_cumul_qty'].forEach(id => {
...
async function eqSubmitCumulative() {
    ...
    const btn = document.getElementById('eqfc_submit_btn');
    ...
    try {
        ...
        if (resp.status === 'success' || r.ok) {
            ...
            setTimeout(() => eqResetCumulForm(), 2000);
        } else {
            throw new Error(resp.message || 'فشل الحفظ');
        }
    } catch(e) {
...
window.eqInitCumulTab = eqInitCumulTab;
window.eqResetCumulForm = eqResetCumulForm;
window.eqBuildElementsList = eqBuildElementsList;
window.eqFilterCumulElementDropdown = eqFilterCumulElementDropdown;
window.eqSubmitCumulative = eqSubmitCumulative;
window.eqPickFromMapCumul = eqPickFromMapCumul;
window.eqClearCumulElement = eqClearCumulElement;
window.eqSelectCumulElement = eqSelectCumulElement;
...
/* ═══════════════════════════════════════════════════
   3. المنفذ اليومي - المعدات (add_eq_*)
   ═══════════════════════════════════════════════════ */
let add_eqFormEquipmentCount = 0;
let _add_eqAllElements = [];

function add_eqBuildElementsList() {
    _add_eqAllElements = [];
    (categories||[]).forEach(cat => {
        (cat.subitems||[]).forEach(sub => {
            if (!allData[sub.sheetId]) return;
            Object.values(allData[sub.sheetId]).forEach(row => {
                const nameKey = row['ROAD NAME']?'ROAD NAME':row['BLOCK NAME']?'BLOCK NAME':'NAME';
                const name=(row[nameKey]||'').trim(), id=(row['ID']||'').trim();
                if(name&&id) _add_eqAllElements.push({id,name,sheetId:sub.sheetId,subName:sub.name});
            });
        });
    });
}

function add_eqPopulateContractors() {
    const sel = document.getElementById('add_eqf_contractor');
    if (!sel) return;
    const cur = sel.value;
    ...
    const fallback = new Set();
    if (!source.length) {
        Object.values(allData||{}).forEach(sd=>Object.values(sd).forEach(row=>{const c=(row['CONTRACTOR']||'').trim();if(c)fallback.add(c);}));
        Object.keys(window.contractorMap||{}).forEach(n=>{if(n.trim())fallback.add(n.trim());});
    }
...
function add_eqShowElementDropdown() { add_eqBuildElementsList(); add_eqFilterElementDropdown(); }
window.add_eqShowElementDropdown = add_eqShowElementDropdown;

function add_eqFilterElementDropdown() {
    const inp=document.getElementById('add_eqf_element_search');
    const dd=document.getElementById('add_eqf_element_dropdown');
    if(!inp||!dd) return;
    const q=(inp.value||'').trim().toLowerCase();
    const filtered=q?_add_eqAllElements.filter(e=>e.name.toLowerCase().includes(q)||e.id.toLowerCase().includes(q)):_add_eqAllElements;
    if(!filtered.length){ dd.innerHTML='<div class="app-dropdown-empty">لا توجد عناصر مطابقة</div>'; }
    else { dd.innerHTML=filtered.slice(0,60).map(e=>'<div class="app-dropdown-item" onclick="add_eqSelectElement(\''+e.id.replace(/'/g,"\\'")+'\',\''+e.name.replace(/'/g,"\\'")+'\')"><span class="app-dropdown-item__title">'+e.name+'</span><span class="app-dropdown-item__meta">ID: '+e.id+' • '+e.subName+'</span></div>').join(''); }
    dd.style.display='block';
    setTimeout(()=>{ document.addEventListener('click',function _c(ev){if(!dd.contains(ev.target)&&ev.target!==inp)dd.style.display='none';else document.addEventListener('click',_c,{once:true,capture:true});},{once:true,capture:true}); },0);
}
window.add_eqFilterElementDropdown = add_eqFilterElementDropdown;

function add_eqSelectElement(id, name) {
    document.getElementById('add_eqf_element_id').value = id;
    document.getElementById('add_eqf_element_name').value = name;
    document.getElementById('add_eqf_element_search').value = name;
    document.getElementById('add_eqf_element_dropdown').style.display = 'none';
    const info=document.getElementById('add_eqf_element_info');
    document.getElementById('add_eqf_element_info_name').textContent=name;
    document.getElementById('add_eqf_element_info_id').textContent='ID: '+id;
    info.style.display='flex';
    const el=_add_eqAllElements.find(e=>e.id===id&&e.name===name)||_add_eqAllElements.find(e=>e.id===id);
    if(el){
        let matchedSub=null,matchedCat=null;
        (categories||[]).forEach(cat=>cat.subitems.forEach(sub=>{if(sub.sheetId===el.sheetId){matchedSub=sub;matchedCat=cat;}}));
        if(matchedSub&&matchedCat){
            document.getElementById('add_eqf_item_name').value=matchedSub.name;
            document.getElementById('add_eqf_band_sheet').value=matchedSub.sheetId||'';
            document.getElementById('add_eqf_cat_name').value=matchedCat.name||'';
            document.getElementById('add_eqf_cat_id').value=matchedCat.id||'';
            const lbl=document.getElementById('add_eqf_band_display');
            if(lbl){lbl.value=matchedSub.name;lbl.style.color='rgba(255,255,255,0.9)';}
            const group=getGroupForSub(matchedSub.id);
            document.getElementById('add_eqf_group_name').value=group?(group.name||'—'):'—';
            document.getElementById('add_eqf_group_id').value=group?(group.id||''):'';
        }
    }
}
window.add_eqSelectElement = add_eqSelectElement;

function add_eqClearElement() {
    ['add_eqf_element_id','add_eqf_element_name','add_eqf_element_search','add_eqf_item_name','add_eqf_band_sheet','add_eqf_cat_name','add_eqf_cat_id','add_eqf_group_name','add_eqf_group_id'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
    const info=document.getElementById('add_eqf_element_info'); if(info)info.style.display='none';
    const lbl=document.getElementById('add_eqf_band_display'); if(lbl){lbl.value='يُملأ تلقائياً عند اختيار العنصر';lbl.style.color='';}
}
window.add_eqClearElement = add_eqClearElement;

function add_eqGetUsedTypes() {
    const used=new Set();
    document.querySelectorAll('#add_eqf_equipments_container .eq-type-inp').forEach(sel=>{if(sel.value)used.add(sel.value);});
    return used;
}

function add_eqRefreshAllSelects() {
    const used=add_eqGetUsedTypes();
    document.querySelectorAll('#add_eqf_equipments_container .eq-type-inp').forEach(sel=>{
        const ownVal=sel.value;
        Array.from(sel.options).forEach(opt=>{if(!opt.value)return;opt.hidden=used.has(opt.value)&&opt.value!==ownVal;});
    });
}

function add_eqAddEquipmentRow() {
    const container=document.getElementById('add_eqf_equipments_container');
    const hint=container.querySelector('.eq-empty-hint'); if(hint)hint.remove();
    if(!equipmentTypes.length){showAlert('❌ لا توجد أنواع معدات — أضفها من الإعدادات');return;}
    const used=add_eqGetUsedTypes();
    const available=equipmentTypes.filter(t=>!used.has(t));
    if(!available.length){showAlert('⚠️ تم اختيار جميع أنواع المعدات المتاحة');return;}
    add_eqFormEquipmentCount++;
    const rowId='add_eqrow_'+add_eqFormEquipmentCount;
    const optionsHtml='<option value="" disabled selected>-- اختر نوع المعدة --</option>'+equipmentTypes.map(t=>'<option value="'+t+'"'+(used.has(t)?' hidden':'')+'>'+t+'</option>').join('');
    const row=document.createElement('div');
    row.className='eq-item-row'; row.id=rowId;
    row.innerHTML='<select class="eq-type-inp" id="'+rowId+'_type">'+optionsHtml+'</select><input type="number" placeholder="العدد" min="0" id="'+rowId+'_count" style="text-align:center;"><button class="eq-del-row-btn" onclick="add_eqRemoveEquipmentRow(\''+rowId+'\')" title="حذف">✕</button>';
    container.appendChild(row);
    const sel=row.querySelector('.eq-type-inp');
    if(sel) sel.addEventListener('change',()=>add_eqRefreshAllSelects());
}
add_eqAddEquipmentRow._userTriggered = false;
window.add_eqAddEquipmentRow = add_eqAddEquipmentRow;

function add_eqRemoveEquipmentRow(rowId) {
    const row=document.getElementById(rowId); if(row)row.remove();
    const container=document.getElementById('add_eqf_equipments_container');
    if(!container.querySelector('.eq-item-row')&&!container.querySelector('.eq-empty-hint')){
        container.innerHTML='<div class="eq-empty-hint">اضغط "إضافة معدة" لإضافة نوع معدة</div>';
        add_eqFormEquipmentCount=0;
    }
    add_eqRefreshAllSelects();
}
window.add_eqRemoveEquipmentRow = add_eqRemoveEquipmentRow;

function add_eqResetForm() {
    ['add_eqf_element_id','add_eqf_element_name','add_eqf_element_search','add_eqf_item_name','add_eqf_band_sheet','add_eqf_cat_name','add_eqf_cat_id','add_eqf_group_name','add_eqf_group_id','add_eqf_contractor','add_eqf_done_qty','add_eqf_row_index','add_eqf_existing_photo_url'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
    const info=document.getElementById('add_eqf_element_info');if(info)info.style.display='none';
    const lbl=document.getElementById('add_eqf_band_display');if(lbl){lbl.value='يُملأ تلقائياً عند اختيار العنصر';lbl.style.color='';}
    const today=new Date().toISOString().split('T')[0];
    const d=document.getElementById('add_eqf_date');if(d)d.value=today;
    const container=document.getElementById('add_eqf_equipments_container');
    if(container){container.innerHTML='<div class="eq-empty-hint">اضغط "إضافة معدة" لإضافة نوع معدة</div>';add_eqFormEquipmentCount=0;}
    const fb=document.getElementById('add_eqf_feedback');if(fb)fb.style.display='none';
}
window.add_eqResetForm = add_eqResetForm;

function add_eqShowFeedback(msg, type) {
    const fb=document.getElementById('add_eqf_feedback');
    if(!fb)return;
    fb.className='eqf-'+type; fb.textContent=msg; fb.style.display='block';
    if(type==='success')setTimeout(()=>{ if(fb)fb.style.display='none'; },4000);
}

async function add_eqSubmitForm() {
    add_eqShowFeedback && add_eqShowFeedback('','');
    const fb=document.getElementById('add_eqf_feedback');if(fb)fb.style.display='none';
    const element_id=document.getElementById('add_eqf_element_id').value.trim();
    const element_name=document.getElementById('add_eqf_element_name').value.trim();
    const item_name=document.getElementById('add_eqf_item_name').value.trim();
    const cat_name=document.getElementById('add_eqf_cat_name').value.trim();
    const group_name=document.getElementById('add_eqf_group_name').value.trim();
    const contractor=document.getElementById('add_eqf_contractor').value.trim();
    const date=document.getElementById('add_eqf_date').value.trim();
    const done_qty=parseFloat(document.getElementById('add_eqf_done_qty').value)||0;
    const band_sheet=document.getElementById('add_eqf_band_sheet').value.trim();
    if(!element_name){add_eqShowFeedback('❌ يرجى اختيار اسم العنصر','error');return;}
    if(!item_name){add_eqShowFeedback('❌ يرجى اختيار البند','error');return;}
    if(!contractor){add_eqShowFeedback('❌ يرجى اختيار المقاول','error');return;}
    if(!date){add_eqShowFeedback('❌ يرجى اختيار التاريخ','error');return;}
    if(!band_sheet){add_eqShowFeedback('❌ البند المختار ليس له شيت مرتبط','error');return;}
    // جمع المعدات
    const rows2=document.querySelectorAll('#add_eqf_equipments_container .eq-item-row');
    const equipments=[];let hasErr=false;
    rows2.forEach(row=>{
        const typeSel=row.querySelector('.eq-type-inp');
        const cntInp=row.querySelector('input[type="number"]');
        const t=typeSel?typeSel.value.trim():'';
        const raw=cntInp?cntInp.value.trim():'';
        const c=parseInt(raw)||0;
        if(t){if(!raw||c<=0){hasErr=true;if(cntInp){cntInp.style.borderColor='#e53935';setTimeout(()=>{cntInp.style.borderColor='';},3000);}}else equipments.push({type:t,count:c});}
    });
    if(hasErr){add_eqShowFeedback('❌ يرجى إدخال عدد صحيح لجميع المعدات','error');return;}
    if(!equipments.length){add_eqShowFeedback('❌ يرجى إضافة معدة واحدة على الأقل','error');return;}
    // جلب scriptUrl
    const allSubs2=(categories||[]).flatMap(c=>c.subitems||[]);
    const matchedSub2=allSubs2.find(s=>s.sheetId===band_sheet);
    const scriptUrl=(matchedSub2&&matchedSub2.scriptUrl)?matchedSub2.scriptUrl.trim():'';
    if(!scriptUrl){add_eqShowFeedback('❌ لم يتم العثور على رابط السكريبت — راجع إعدادات البند','error');return;}
    const btn=document.getElementById('add_eqf_submit_btn');
    btn.disabled=true;btn.textContent='⏳ جاري الحفظ...';
    add_eqShowFeedback('⏳ جاري إرسال البيانات...','loading');
    const row_index=parseInt(document.getElementById('add_eqf_row_index')?.value)||null;
    const added_by=(currentUser&&currentUser.email)?currentUser.email:(currentUser&&currentUser.name?currentUser.name:'');
    const photo_url=(document.getElementById('add_eqf_existing_photo_url')?.value||'');
    const payload={form_type:'daily',row_index,group_name,cat_name,element_id,element_name,item_name,contractor,date,done_qty,equipments,added_by,photo_url};
    try {
        const r=await fetch(scriptUrl,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify(payload),redirect:'follow'});
        const text=await r.text(); let resp={};
        try{resp=JSON.parse(text);}catch(e){}
        if(resp.status==='success'||r.ok){add_eqShowFeedback('✅ تم حفظ بيانات المعدات بنجاح!','success');showAlert('✅ تم تسجيل الكمية - المعدات بنجاح','success');setTimeout(()=>add_eqResetForm(),2500);}
        else throw new Error(resp.message||'فشل الحفظ');
    } catch(e) {
        add_eqShowFeedback('❌ تعذر الحفظ: '+(e.message||'خطأ'),'error');
    } finally {
        btn.disabled=false;btn.textContent='💾 حفظ في السجل';
    }
}
window.add_eqSubmitForm = add_eqSubmitForm;

/* ── اختيار من الخريطة ── */
let _add_eqPickingFromMap=false, _add_eqMapClickHandler=null, _add_eqMapBgClickHandler=null;

function add_eqPickFromMap() {
    if(!map){showAlert('❌ الخريطة غير جاهزة');return;}
    if(!Object.keys(allLayers).length){showAlert('❌ حمّل بنداً على الخريطة أولاً');return;}
    _add_eqPickingFromMap=true;
    const scr=document.getElementById('additionScreen');if(scr)scr.style.setProperty('display','none','important');
    let hint=document.getElementById('add_eqPickMapHint');
    if(!hint){hint=document.createElement('div');hint.id='add_eqPickMapHint';hint.style.cssText='position:fixed;top:70px;left:50%;transform:translateX(-50%);z-index:99999;background:linear-gradient(135deg,#1a4a8a,#2196f3);color:white;padding:12px 24px;border-radius:12px;font-size:13px;font-weight:700;font-family:Cairo,sans-serif;box-shadow:0 8px 28px rgba(33,150,243,0.5);display:flex;align-items:center;gap:14px;white-space:nowrap;pointer-events:auto;';hint.innerHTML='<span>🗺 انقر على أي عنصر لاختياره</span><button onclick="add_eqCancelPickFromMap()" style="background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.3);color:white;padding:4px 12px;border-radius:7px;font-size:12px;font-weight:700;font-family:Cairo,sans-serif;cursor:pointer;">إلغاء</button>';document.body.appendChild(hint);}
    hint.style.display='flex';map.getContainer().style.cursor='crosshair';
    _add_eqMapClickHandler=function(e){
        if(!_add_eqPickingFromMap)return;
        if(e.originalEvent){e.originalEvent.stopPropagation();e.originalEvent.preventDefault();}
        if(map.closePopup)map.closePopup();
        const f=e.target||e.layer;let row=null;
        if(f&&f.feature){const fid=f.feature.properties.ID;for(const[sid,data]of Object.entries(allData)){if(data[fid]){row=data[fid];break;}}}
        add_eqCancelPickFromMap();
        if(row){const nk=row['ROAD NAME']?'ROAD NAME':row['BLOCK NAME']?'BLOCK NAME':'NAME';const name=(row[nk]||'').trim()||row['ID'];add_eqSelectElement(row['ID']||'',name);showAlert('✅ تم اختيار: '+name,'success');}
    };
    Object.values(allLayers).forEach(layer=>{if(layer)layer.eachLayer(f=>f.on('click',_add_eqMapClickHandler));});
    map.on('click',_add_eqMapBgClickHandler=function(e){});
}
window.add_eqPickFromMap = add_eqPickFromMap;

function add_eqCancelPickFromMap() {
    _add_eqPickingFromMap=false;
    const scr=document.getElementById('additionScreen');if(scr){scr.style.removeProperty('display');scr.style.display='flex';}
    const hint=document.getElementById('add_eqPickMapHint');if(hint)hint.style.display='none';
    if(map)map.getContainer().style.cursor='';
    if(_add_eqMapClickHandler){Object.values(allLayers).forEach(layer=>{if(layer)layer.eachLayer(f=>f.off('click',_add_eqMapClickHandler));});_add_eqMapClickHandler=null;}
    if(map&&_add_eqMapBgClickHandler){map.off('click',_add_eqMapBgClickHandler);_add_eqMapBgClickHandler=null;}
    if(map)map.closePopup();
}
window.add_eqCancelPickFromMap = add_eqCancelPickFromMap;

/* ═══════════════════════════════════════════════════
   4. تهيئة التبويبات عند الفتح (additionTab:changed)
   ═══════════════════════════════════════════════════ */
window.addEventListener('additionTab:changed', function(e) {
    const tab = e.detail && e.detail.tab;

    if (tab === 'cashco') {
        // تهيئة التاريخ
        const d = document.getElementById('add_ccf_date');
        if (d && !d.value) d.value = new Date().toISOString().split('T')[0];
        // تحميل رقم المستخلص والسجل
        add_ccfLoad();
    }

    if (tab === 'cashctr') {
        const d = document.getElementById('add_concf_date');
        if (d && !d.value) d.value = new Date().toISOString().split('T')[0];
        add_concfPopulateContractors();
    }

    if (tab === 'equipment') {
        add_eqBuildElementsList();
        add_eqPopulateContractors();
        const today = new Date().toISOString().split('T')[0];
        const d = document.getElementById('add_eqf_date');
        if (d && !d.value) d.value = today;
        // أضف صف معدة فارغ لو القائمة فارغة
        const container = document.getElementById('add_eqf_equipments_container');
        if (container && !container.querySelector('.eq-item-row')) {
            container.innerHTML = '<div class="eq-empty-hint">اضغط "إضافة معدة" لإضافة نوع معدة</div>';
            add_eqFormEquipmentCount = 0;
        }
    }
...
/* تهيئة فورية لو فُتحت الشاشة على التبويب مباشرة */
window.addEventListener('load', function() {
    // تهيئة أولية للتواريخ
    const today = new Date().toISOString().split('T')[0];
    ['add_ccf_date','add_concf_date','add_eqf_date'].forEach(id => {
        const el = document.getElementById(id);
        if (el && !el.value) el.value = today;
    });
    // تهيئة فورية لأي تبويب نشط
    const activePanel = document.querySelector('#additionScreen .add-tab-panel.active[data-tab]');
    if (activePanel) {
        const t = activePanel.dataset.tab;
        if (t === 'equipment') { add_eqBuildElementsList(); add_eqPopulateContractors(); }
        if (t === 'cashctr')   { add_concfPopulateContractors(); }
        if (t === 'cashco')    { add_ccfLoad(); }
    }
    });
/* ══════════════════════════════════════════════════
   9. تصدير للـ window
...
// ── تصدير الدوال الناقصة على window ──
window.openEquipmentFormModal      = openEquipmentFormModal;
window.closeEquipmentFormModal     = closeEquipmentFormModal;
window.eqAddEquipmentRow           = eqAddEquipmentRow;
window.eqRemoveEquipmentRow        = eqRemoveEquipmentRow;
window.eqSubmitForm                = eqSubmitForm;
window.eqResetForm                 = eqResetForm;
window.eqShowFeedback              = eqShowFeedback;
window.eqHideFeedback              = eqHideFeedback;
window.eqShowEmptyHint             = eqShowEmptyHint;
window.eqCollectEquipments         = eqCollectEquipments;
window.eqRefreshAllSelects         = eqRefreshAllSelects;
window.eqGetUsedTypes              = eqGetUsedTypes;
window.eqPopulateSubitems          = eqPopulateSubitems;
window.eqPopulateContractors       = eqPopulateContractors;
window.eqShowElementDropdown       = eqShowElementDropdown;
window.eqFilterElementDropdown     = eqFilterElementDropdown;
window.eqSelectElement             = eqSelectElement;
window.eqClearElement              = eqClearElement;
window.eqPickFromMap               = eqPickFromMap;
window.eqCancelPickFromMap         = eqCancelPickFromMap;
window.eqOpenBandPicker            = eqOpenBandPicker;
window.eqCloseBandPicker           = eqCloseBandPicker;
window.eqFilterBandPicker          = eqFilterBandPicker;
window.eqRenderBandPicker          = eqRenderBandPicker;
window.eqSelectBand                = eqSelectBand;
window.eqSwitchFormTab             = eqSwitchFormTab;
window.add_eqBuildElementsList     = add_eqBuildElementsList;
window.add_eqPopulateContractors   = add_eqPopulateContractors;
window.add_eqGetUsedTypes          = add_eqGetUsedTypes;
window.add_eqRefreshAllSelects     = add_eqRefreshAllSelects;
window.add_eqShowFeedback          = add_eqShowFeedback;
