...
function loadLayer(sheetId, subitemName, geoJsonFile, catId) {
    ...
    fetch(csvUrl)
        ...
        .then(csv => {
            ...
            const isWanted = () =>
                categories.flatMap(c => c.subitems)
                    .some(s => s.sheetId === sheetId && selectedItems[s.id])
                || [...activeContractorFilter]
                    .some(k => k.startsWith(sheetId + '|'));
            ...
            if (idIdx === -1) {
                showAlert("❌ لا يوجد عمود ID في الشيت");
                return;
            }
            ...
            return fetch(geoJsonFile + "?t=" + Date.now())
                .then(r => r.json())
                .then(geo => {
                    ...
                    const layer = L.geoJSON(geo, {
                        onEachFeature: (f, l) => {
                            ...
                            const nameKey =
                                row["ROAD NAME"] ? "ROAD NAME"
                              : row["BLOCK NAME"] ? "BLOCK NAME"
                              : "NAME";
        ...
        .catch(e => {
            console.error(e);
            showAlert("❌ خطأ في تحميل البيانات");
        });
}
    ...
    if (activeContractorFilter) {
        [...activeContractorFilter].forEach(k => {
            if (k.startsWith(sheetId + '|')) {
                activeContractorFilter.delete(k);
            }
        });
    }
