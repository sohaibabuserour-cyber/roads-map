...
function refreshLayerColors() {
    if (window.activeContractorFilter && window.activeContractorFilter.size > 0) {
        if (window.applyContractorFilter) window.applyContractorFilter();
    } else {
    ...
    fetch(csvUrl)
        ...
        .then(csv => {
            ...
            const isWanted = () =>
                ...
                || (window.activeContractorFilter && [...window.activeContractorFilter].some(k => k.startsWith(sheetId + '|')));
            if (!isWanted()) return;
            ...
            return fetch(geoJsonFile + "?t=" + Date.now()).then(r => r.json()).then(geo => {
                    ...
                    onEachFeature: (f, l) => {
                        ...
                        Object.keys(row).forEach(k => {
                            ...
                            html += `<div class="popup-row">
                                <div class="popup-label">${window.LABELS?.[k] || k}</div>
                ...
                if (window.contractorsLoaded) {
                    if (window.renderContractorList) window.renderContractorList();
                } else {
                    if (window.buildContractorPanel) window.buildContractorPanel();
                }
            });
        })
...
function removeLayer(sheetId) {
    ...
    if (window.activeContractorFilter) {
        [...window.activeContractorFilter].forEach(k => { if (k.startsWith(sheetId + '|')) window.activeContractorFilter.delete(k); });
    }
    if (window.updateStats) window.updateStats();
    if (window.renderContractorList) window.renderContractorList();
}